...
class Arena {
    ...
    std::vector<Tile> m_tails, m_init_tails;
...
public:
    ...
    inline uint count() const                            { return m_tails.size(); }

    inline Tile tile(const uint index) const             { return m_tails[index]; }
    inline const std::vector<Tile> & tiles() const       { return m_tails; }
    ...
    inline const std::vector<std::vector<uint>> & neighbors() const { return m_neighbors; }
    inline const std::vector<uint> & neighbors(const uint index) const;
    inline const std::vector<uint> & rev_neighbors(const uint index) const;
...
const std::vector<uint> & Arena::neighbors(const uint index) const {
    return m_neighbors[index];
}

const std::vector<uint> & Arena::rev_neighbors(const uint index) const {
    return m_rev_neighbors[index];
}
}
